...
doThings.config(['$stateProvider', '$locationProvider',
...
doThings.controller('doController', function($scope, $firebaseArray, $firebaseObject) {
  var fireRef = new Firebase("https://blinding-fire-8984.firebaseio.com/work/tasks");
  var fireCol = new Firebase("https://blinding-fire-8984.firebaseio.com/work/colors");
  var fireCss = new Firebase("https://blinding-fire-8984.firebaseio.com/work/colors/css");
  ...
  cssNow.$bindTo($scope, "data").then(function() {
    var currentCss = $scope.data.$value;
  ...
  $scope.showtask = function(taskTime, taskDone, taskType) {
    var oneWeekAgo = new Date().getTime() - 1000 * 60 * 60 * 24 * 7, //one week
      oneDayAgo = new Date().getTime() - 1000 * 60 * 60 * 24, //one day - for testing
      oneHourAgo = new Date().getTime() - 1000 * 60 * 60; //one hour - for testing
    oneYearAgo = new Date().getTime() - 1000 * 60 * 60 * 24 * 365; //one year - for demo app so tasks visible up for viewing
    if ((oneYearAgo < taskTime) && (taskDone !== 'Yes!') && ((taskType == $scope.taskSee) || ($scope.taskSee == 'All'))) return true;
  };
});
